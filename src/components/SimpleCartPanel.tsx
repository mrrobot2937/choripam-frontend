...
export default function SimpleCartPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { cart, removeFromCart, updateQuantity, clearCart, getTotalPrice, restaurantId } = useCart();
  ...
  async function handleSubmitOrder() {
    ...
    try {
      ...
      const orderData = {
        ...
        modalidad_entrega: "Mesa",
        mesa: "1"
      };

      console.log('🔄 Enviando pedido simplificado:', orderData);
      
      const result = await apiService.createOrder(orderData, restaurantId);
      
      if (result.success) {
        console.log('✅ Pedido enviado exitosamente:', result.order_id);
        clearCart();
        onClose();
        alert(`¡Pedido enviado exitosamente! Número de pedido: ${result.order_id}`);
      } else {
        throw new Error(result.message || 'Error enviando el pedido');
      }
    } catch (error) {
      console.error('❌ Error enviando pedido:', error);
      alert(`Error enviando el pedido: ${error instanceof Error ? error.message : 'Error desconocido'}`);
    } finally {
          ...
          <div className="p-6 border-t border-zinc-800 space-y-4">
            ...
            >
              {isSubmitting ? (
                ...
                `Enviar pedido (${cart.length} producto${cart.length !== 1 ? 's' : ''})`
              )}
            </button>
